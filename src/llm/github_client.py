...
class LLMClient(BaseLLMClient):
    ...
    def generate_test(self, prompt: str) -> dict:
        ...
        Returns:
            dict: Il codice Pytest generato, i token usati e le stime energetiche.
        """
        try:
            ...
            # EcoLogits inietta l'oggetto 'impacts' nella risposta dell'SDK OpenAI
            impacts = getattr(response, "impacts", None)
            # Estraiamo i kWh
            cloud_energy = impacts.energy.value if impacts else 0.0
            # gwp (Global Warming Potential) è in kgCO2eq, lo convertiamo in grammi
            cloud_co2 = (impacts.gwp.value * 1000) if impacts else 0.0

            return {
                "code": message.strip(),
                "input_tokens": getattr(usage, "prompt_tokens", 0),
                "output_tokens": getattr(usage, "completion_tokens", 0),
                "cloud_energy_kwh": cloud_energy,
                "cloud_co2_g": cloud_co2
            }
